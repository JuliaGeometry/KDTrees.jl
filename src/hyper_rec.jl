...
function split_hyper_rec{T <: AbstractFloat}(hyper_rec::HyperRectangle{T},
                                             dim::Int,
                                             value::T)
...
############################################
@inline function get_min_dim{T <: AbstractFloat}(rec1::HyperRectangle{T},  rec2::HyperRectangle{T}, dim::Int)
    return abs2(max(0, max(rec1.mins[dim] - rec2.maxes[dim], rec2.mins[dim] - rec1.maxes[dim])))
end

@inline function get_max_dim{T <: AbstractFloat}(rec1::HyperRectangle{T},  rec2::HyperRectangle{T}, dim::Int)
    return abs2(max(rec1.maxes[dim] - rec2.mins[dim],  rec2.maxes[dim] - rec1.mins[dim]))
end

@inline function get_min_max_distance{T <: AbstractFloat}(rec1::HyperRectangle{T}, rec2::HyperRectangle{T})
    min_dist = get_min_distance(rec1, rec2)
    max_dist = get_max_distance(rec1, rec2)
    return min_dist, max_dist
end

@inline function get_min_distance{T <: AbstractFloat}(rec1::HyperRectangle{T}, rec2::HyperRectangle{T})
    min_dist = zero(T)
...
@inline function get_max_distance{T <: AbstractFloat}(rec1::HyperRectangle{T}, rec2::HyperRectangle{T})
    max_dist = zero(T)
    @inbounds for dim in 1:length(rec1.maxes)
...
@inline function get_min_max_dim{T <: AbstractFloat}(rec1::HyperRectangle{T},  rec2::HyperRectangle{T}, dim::Int)
